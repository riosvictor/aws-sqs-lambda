...
process.env.QUEUE_URL = 'http://localhost:4566/000000000000/minha-fila';
...
describe('producer handler', () => {
  ...
  test('envia mensagem ao SQS com a QueueUrl correta', async () => {
  ...
  test('serializa o body do evento como MessageBody', async () => {
    ...
    assert.deepEqual(JSON.parse(cmd.input['MessageBody']), payload);
  });
